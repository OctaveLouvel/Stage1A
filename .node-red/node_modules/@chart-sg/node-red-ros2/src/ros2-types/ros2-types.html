...
<script type="text/javascript">
    RED.nodes.registerType('ROS2 Type',
    {
        ...
        oneditprepare: function()
        {
            ...
            console.log(this.ros2pkg);
            console.log(this.ros2message);
            if (this.ros2pkg)
            {
            ...
            $("#node-input-ros2package-select").mousedown(function () {
                if (!$("#node-input-ros2package-select").is(":focus"))
                {
                    ...
                    $.getJSON('ros2msgspackages', function(data)
                    {
                        $("#node-input-ros2package-select").empty();
        ...
        oneditsave: function()
        {
            ...
            console.log(node.ros2pkg);
            console.log(node.ros2message);
            ...
            RED.nodes.eachLink( function(n)
            {
                if (n.source.id == node.id)
                {
                    switch(n.target.type)
                    {
                        case "Publisher":
                        case "Subscriber":
                            const selectedtype = node.ros2pkg + '/msg/' + node.ros2message;
                            if (selectedtype !== '/')
                            {
        ...
        if (link.source.type == 'ROS2 Type')
        {
            switch(link.target.type)
            {
                ...
                case "FIWARE Subscriber":
                    const selectedtype = link.source.ros2pkg + '/msg/' + link.source.ros2message;
                    if (selectedtype !== '/')
                    {
